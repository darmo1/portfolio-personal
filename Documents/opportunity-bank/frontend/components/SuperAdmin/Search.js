...
const Search = ({ IdSearched, dispatch, setLoading }) => {
  const handleSearch = async () => {
    if (IdSearched) {
      ...
      try {
        ...
        if (res.status === 200) {
          dispatch({ dataUser: [dataUser], messageRol: '', statusRol: dataUser.rolID })
          setLoading(false)
        }
      } catch (err) {
        setLoading(false)
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-between md:flex-row w-full mx-auto md:w-4/5">
